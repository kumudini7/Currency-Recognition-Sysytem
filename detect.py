...
test_image_name = 'test_2000_1.jpg'
...
def preprocess(img, showImages = True):
    # showImages and display('Before Processing', img) 1

    img = resize_img(img, 0.7)
    # showImages and display('After Resize', img) 2
    # showImages and cv2.imwrite('1_after_resize.jpg',img)

    img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    # showImages and display('After Grayscale', img) 3
    ...
    # showImages and display('After Bilateral Blur', img) 4
    # showImages and cv2.imwrite('3_after_blur.jpg',img)

    ret2,img = cv2.threshold(img,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
    # showImages and display('After Otsu thresholding', img) 5
    # showImages and cv2.imwrite('4_after_thresold.jpg',img)

    kernel = np.ones((3,3),np.uint8)
    img = cv2.morphologyEx(img, cv2.MORPH_OPEN, kernel, iterations=2)
    # showImages and display('After Morphological Processing', img) 6
